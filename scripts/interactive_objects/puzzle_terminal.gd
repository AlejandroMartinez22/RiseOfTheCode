...
# Con sistema de persistencia y recompensas múltiples
extends Area2D
...
# ==================== DATOS DEL PUZZLE ====================
@export_group("Etapas del Puzzle")
@export var puzzle_stages: Array[Dictionary] = []
# Formato de cada Dictionary:
# {
#   "question": "¿Pregunta?",
#   "options": [
#     {"text": "[A] Opción 1", "is_correct": false, "feedback": "Feedback incorrecto"},
#     {"text": "[B] Opción 2", "is_correct": true, "feedback": "¡Correcto!"}
#   ]
# }
...
func open_puzzle() -> void:
	...
	# Preparar datos del puzzle
	var puzzle_data = {
		"puzzle_id": puzzle_id,
		"background_texture": background_texture,
		"stages": puzzle_stages
	}
...
func grant_all_rewards() -> void:
	# Ejecutar todas las recompensas habilitadas
...
func grant_unlock_door_reward() -> void:
	...
	if door and door.has_method("unlock"):
		...
		# Registrar para cuando se cargue la puerta
...
func _get_configuration_warnings() -> PackedStringArray:
	...
	if puzzle_stages.is_empty():
		warnings.append("No hay etapas configuradas en 'puzzle_stages'")
